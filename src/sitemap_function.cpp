...
struct SitemapCache {
  ...
  void Set(const std::string &base_url,
           const std::vector<std::string> &sitemaps) {
    std::lock_guard<std::mutex> lock(cache_mutex);
...
static std::string BuildUrl(const std::string &base_url,
...
static void FetchSitemap(ClientContext &context, const std::string &sitemap_url,
                         SitemapGlobalState &state,
                         const SitemapBindData &bind_data, int current_depth) {
  if (current_depth > bind_data.max_depth) {
    return; // Prevent infinite recursion
  }

  auto response = HttpClient::Fetch(
      context, sitemap_url, bind_data.retry_config, bind_data.user_agent);

  if (!response.success) {
    std::lock_guard<std::mutex> lock(state.mutex);
    state.errors.push_back("Failed to fetch " + sitemap_url + ": " +
                           response.error);
    return;
  }
  ...
  if (XmlParser::IsGzipped(sitemap_url, response.content_type)) {
    ...
    if (content.empty()) {
      std::lock_guard<std::mutex> lock(state.mutex);
      state.errors.push_back("Failed to decompress gzipped sitemap: " +
                             sitemap_url);
      return;
    }
  }
  ...
  if (!result.success) {
    std::lock_guard<std::mutex> lock(state.mutex);
    state.errors.push_back("Failed to parse sitemap " + sitemap_url + ": " +
                           result.error);
    return;
  }
  ...
  if (result.type == SitemapType::URLSET) {
    // Add URLs to state
    std::lock_guard<std::mutex> lock(state.mutex);
    state.entries.insert(state.entries.end(), result.urls.begin(),
                         result.urls.end());
  } else {
...
static unique_ptr<FunctionData> SitemapBind(ClientContext &context,
  ...
  if (first_param.type().id() == LogicalTypeId::VARCHAR) {
    ...
    throw InvalidInputException(
        "sitemap_urls() first argument must be VARCHAR or LIST(VARCHAR)");
  }
...
DiscoverSitemapUrls(ClientContext &context, const std::string &base_url,
  ...
  if (bind_data.follow_robots) {
    ...
    auto response = HttpClient::Fetch(
        context, robots_url, bind_data.retry_config, bind_data.user_agent);
  ...
  auto sitemap_response = HttpClient::Fetch(
      context, sitemap_xml_url, bind_data.retry_config, bind_data.user_agent);
  ...
  auto index_response = HttpClient::Fetch(
      context, sitemap_index_url, bind_data.retry_config, bind_data.user_agent);
  ...
  auto html_response = HttpClient::Fetch(
      context, homepage_url, bind_data.retry_config, bind_data.user_agent);
...
SitemapInitGlobal(ClientContext &context, TableFunctionInitInput &input) {
  ...
  for (const auto &base_url : bind_data.base_urls) {
    ...
    std::vector<std::string> sitemap_urls =
        DiscoverSitemapUrls(context, base_url, bind_data);
...
SitemapInitLocal(ExecutionContext &context, TableFunctionInitInput &input,
                 GlobalTableFunctionState *global_state) {
  return make_uniq<SitemapLocalState>();
}

// Scan function - return entries in batches
static void SitemapScan(ClientContext &context, TableFunctionInput &data,
                        DataChunk &output) {
  ...
  while (count < max_count && state.current_idx < state.entries.size()) {
    ...
    output.SetValue(1, count,
                    entry.lastmod.empty() ? Value() : Value(entry.lastmod));
    output.SetValue(
        2, count, entry.changefreq.empty() ? Value() : Value(entry.changefreq));
    output.SetValue(3, count,
                    entry.priority.empty() ? Value() : Value(entry.priority));
...
void RegisterSitemapFunction(ExtensionLoader &loader) {
  ...
  TableFunction sitemap_func("sitemap_urls", {LogicalType::VARCHAR},
                             SitemapScan, SitemapBind, SitemapInitGlobal);
  ...
  TableFunction sitemap_func_list("sitemap_urls",
