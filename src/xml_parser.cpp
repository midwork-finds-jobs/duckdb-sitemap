...
void XmlParser::Initialize() { xmlInitParser(); }

// Cleanup libxml2 (optional, for clean shutdown)
void XmlParser::Cleanup() { xmlCleanupParser(); }
...
XMLDocRAII::XMLDocRAII(const std::string &content) {
  ...
  doc = xmlReadMemory(
      content.c_str(), static_cast<int>(content.size()), nullptr, nullptr,
...
static std::string GetXPathText(xmlXPathContextPtr ctx, xmlNodePtr node,
                                const char *xpath) {
  // Set context node
  ...
  xmlXPathRegisterNs(local_ctx, BAD_CAST "sm",
                     BAD_CAST "http://www.sitemaps.org/schemas/sitemap/0.9");
  xmlXPathRegisterNs(local_ctx, BAD_CAST "sm2",
                     BAD_CAST "http://www.google.com/schemas/sitemap/0.84");
...
SitemapParseResult XmlParser::ParseSitemap(const std::string &xml_content) {
  ...
  if (root_name == "sitemapindex") {
    ...
    const char *xpath_variants[] = {"//sm:sitemap/sm:loc",
                                    "//sm2:sitemap/sm2:loc"};
    for (const char *xpath : xpath_variants) {
      xmlXPathObjectPtr sitemap_nodes =
          xmlXPathEvalExpression(BAD_CAST xpath, doc.xpath_ctx);

      if (sitemap_nodes && sitemap_nodes->nodesetval &&
          sitemap_nodes->nodesetval->nodeNr > 0) {
    ...
    for (int ns_idx = 0; ns_idx < 2; ns_idx++) {
      xmlXPathObjectPtr url_nodes = xmlXPathEvalExpression(
          BAD_CAST xpath_variants[ns_idx], doc.xpath_ctx);

      if (url_nodes && url_nodes->nodesetval &&
          url_nodes->nodesetval->nodeNr > 0) {
        for (int i = 0; i < url_nodes->nodesetval->nodeNr; i++) {
          xmlNodePtr url_node = url_nodes->nodesetval->nodeTab[i];

          SitemapEntry entry;
          entry.url =
              GetXPathText(doc.xpath_ctx, url_node, loc_variants[ns_idx]);
          entry.lastmod =
              GetXPathText(doc.xpath_ctx, url_node, lastmod_variants[ns_idx]);
          entry.changefreq = GetXPathText(doc.xpath_ctx, url_node,
                                          changefreq_variants[ns_idx]);
          entry.priority =
              GetXPathText(doc.xpath_ctx, url_node, priority_variants[ns_idx]);
...
bool XmlParser::IsGzipped(const std::string &url,
...
XmlParser::FindSitemapInHtml(const std::string &html_content) {
  std::vector<std::string> sitemaps;

  // Parse HTML with libxml2 in HTML mode
  xmlDocPtr doc = htmlReadMemory(
      html_content.c_str(), static_cast<int>(html_content.size()), nullptr,
  ...
  xmlXPathObjectPtr link_nodes =
      xmlXPathEvalExpression(BAD_CAST link_xpath, xpath_ctx);
