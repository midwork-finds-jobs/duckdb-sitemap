...
HttpResponse HttpClient::ExecuteHttpGet(DatabaseInstance &db,
  ...
  if (!user_agent.empty()) {
    ...
    query = StringUtil::Format("SELECT status, decode(body) AS body, "
                               "content_type, "
                               "headers['retry-after'] AS retry_after "
                               "FROM http_get('%s')",
                               escaped_url);
...
HttpResponse HttpClient::Fetch(ClientContext &context, const std::string &url,
  ...
  for (int attempt = 0; attempt <= config.max_retries; attempt++) {
    ...
    if (response.status_code == 429 && !response.retry_after.empty()) {
      ...
      if (wait_ms <= 0) {
        ...
        wait_ms =
            static_cast<int>(config.initial_backoff_ms *
                             std::pow(config.backoff_multiplier, attempt));
      }
    } else {
      // Exponential backoff
      wait_ms = static_cast<int>(config.initial_backoff_ms *
                                 std::pow(config.backoff_multiplier, attempt));
    }
