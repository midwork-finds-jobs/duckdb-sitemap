...
static void LoadInternal(ExtensionLoader &loader) {
  ...
  config.AddExtensionOption("sitemap_user_agent",
                            ...
                            LogicalType::VARCHAR, Value("DuckDB-Sitemap/1.0"));
  ...
  if (install_result->HasError()) {
    throw IOException("Sitemap extension requires http_request extension. "
                      "Failed to install: " +
  ...
  if (load_result->HasError()) {
    throw IOException(
        "Sitemap extension requires http_request extension. Failed to load: " +
...
void SitemapExtension::Load(ExtensionLoader &loader) { LoadInternal(loader); }

std::string SitemapExtension::Name() { return "sitemap"; }
...
DUCKDB_CPP_EXTENSION_ENTRY(sitemap, loader) { duckdb::LoadInternal(loader); }
}
