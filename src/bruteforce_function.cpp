...
static void BruteforceFindSitemapFunction(DataChunk &args,
  ...
  retry_config.max_retries =
      0; // No retries for bruteforce (too many URLs to check)
  ...
  for (idx_t i = 0; i < args.size(); i++) {
    ...
    for (const auto &filename : filenames) {
      for (const auto &filetype : filetypes) {
        ...
        auto response =
            HttpClient::Fetch(context, url, retry_config, user_agent);
...
void RegisterBruteforceFunction(ExtensionLoader &loader) {
  ScalarFunction bruteforce_func("bruteforce_find_sitemap",
